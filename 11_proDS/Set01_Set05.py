...
q1 = df[['Sex', 'BP', 'Cholesterol']].value_counts(normalize=True)
...
# 변수 생성 : np.where(조건 [, 참 실행문, 거짓 실행문])
...
var_ls = ['Sex', 'BP', 'Cholesterol', 'Age_gr', 'Na_K_gr']
for i in var_ls:
...
# 연관성 있는 변수 가운데 가장 큰 p-value 찾아 소수점 6자리 이하는 버리기
round(q2_out2.pvalue.max(),6)
...
q3['Sex_cd'] = np.where(q3.Sex == 'F', 1, 0)
q3['BP_cd'] = np.where(q3.BP == 'LOW', 0, np.where(q3.BP == 'NORMAL', 1, 2))
q3['Ch_cd'] = np.where(q3.Cholesterol == 'NORMAL', 0,1)

from sklearn.tree import DecisionTreeClassifier, plot_tree, export_text

dt = DecisionTreeClassifier().fit(q3[var_ls], q3['Drug'])

plot_tree(dt, feature_names=var_ls ,class_names=(q3['Drug'].unique()), max_depth=1, fontsize=8)


print(export_text(dt, feature_names=var_ls, decimals=3))
